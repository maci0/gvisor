...
| Guest mode | EL0 (user) + EL1 (kernel) | EL0 (user) + EL1 (vectors/stubs) |
| Page tables | Managed by KVM (EPT/NPT) | Stage 1: per-MM in userspace; Stage 2: HVF |
| Syscall trap | SVC from EL0 -> EL1 vectors | SVC from EL0 -> EL1 vectors -> HVC exit |
...
sync vector (offset 0x400) to an HVC instruction that exits to the hypervisor.
...
The entry stub runs at EL1 (CPSR=0x3C5) to execute TLBI VMALLE1IS, then ERETing
...
entries from previous runs become invalid, ensuring new page table entries
are visible. Additionally, the entry stub executes TLBI VMALLE1IS on every
guest entry for full TLB coherency.
