...
// Package rawfile contains utilities for using raw host files on Darwin hosts.
package rawfile
...
// On macOS, sendmmsg is not available, so we send one at a time using
// sendmsg.
func NonBlockingSendMMsg(fd int, msgHdrs []MMsgHdr) (int, unix.Errno) {
	for i := range msgHdrs {
		_, _, e := unix.RawSyscall(unix.SYS_SENDMSG, uintptr(fd), uintptr(unsafe.Pointer(&msgHdrs[i].Msg)), uintptr(unix.MSG_DONTWAIT))
		if e != 0 {
			if i > 0 {
				return i, 0
			}
		...
		// TODO(gvisor.dev/issue/): set msgHdrs[i].Len from sendmsg return value.
	}
	return len(msgHdrs), 0
}
...
//
// On macOS, recvmmsg is not available, so we receive one message at a time
// using recvmsg.
func BlockingRecvMMsgUntilStopped(efd int, fd int, msgHdrs []MMsgHdr) (int, unix.Errno) {
	for {
		// Try to receive one message.
		n, _, e := unix.RawSyscall(unix.SYS_RECVMSG, uintptr(fd), uintptr(unsafe.Pointer(&msgHdrs[0].Msg)), uintptr(unix.MSG_DONTWAIT))
		if e == 0 {
			msgHdrs[0].Len = uint32(n)
			return 1, 0
		}
...
// On macOS, this uses poll(2) instead of ppoll(2).
func BlockingPollUntilStopped(efd int, fd int, events int16) (bool, unix.Errno) {
	...
	// Use poll(2) via BlockingPoll. Pass nil timeout to block indefinitely.
