...
//  2. O_SYMLINK: for symlinks (macOS doesn't have O_PATH).
//     Note: O_SYMLINK must NOT be combined with O_NOFOLLOW on macOS,
//     as that combination returns ELOOP.
func tryOpen(open func(int) (int, error)) (int, error) {
	// First try O_RDONLY|O_NONBLOCK: works for regular files, directories, FIFOs.
	hostFD, err := open(unix.O_RDONLY | unix.O_NONBLOCK | hostOpenFlags)
	if err == nil {
		return hostFD, nil
	}
	if err == unix.ENOENT {
	...
	// O_SYMLINK for symlinks. Must NOT combine with O_NOFOLLOW (ELOOP on macOS).
	hostFD, err = open(unix.O_SYMLINK | unix.O_CLOEXEC)
	if err == nil {
		return hostFD, nil
	}
	return -1, err
...
func (i *directfsInode) prepareSetStat(ctx context.Context, stat *linux.Statx, d *dentry) error {
	if stat.Mask&linux.STATX_SIZE != 0 ||
		(stat.Mask&(linux.STATX_ATIME|linux.STATX_MTIME) != 0 && i.isRegularFile()) {
...
func (i *directfsInode) setStatLocked(ctx context.Context, stat *linux.Statx, d *dentry) (failureMask uint32, failureErr error) {
	...
	if stat.Mask&linux.STATX_SIZE != 0 {
		...
		if err := unix.Ftruncate(int(i.writeFD.RacyLoad()), int64(stat.Size)); err != nil {
			failureMask |= linux.STATX_SIZE
			failureErr = err
		}
	}

	if err := i.utimensat(ctx, stat, d); err != nil {
		failureMask |= (stat.Mask & (linux.STATX_ATIME | linux.STATX_MTIME))
		failureErr = err
	}
	...
	if stat.Mask&(linux.STATX_UID|linux.STATX_GID) != 0 {
		...
		if err := fchown(i.controlFD, uid, gid); err != nil {
			failureMask |= stat.Mask & (linux.STATX_UID | linux.STATX_GID)
			failureErr = err
		}
	}
	return
...
func fchown(fd int, uid auth.KUID, gid auth.KGID) error {
	...
	// macOS doesn't support AT_EMPTY_PATH. Use fchown(2) directly.
	return unix.Fchown(fd, u, g)
}
...
func (i *directfsInode) listXattr(ctx context.Context, size uint64, d *dentry) ([]string, error) {
	...
	data := make([]byte, size)
	n, err := unix.Flistxattr(i.controlFD, data)
	if err != nil {
		return nil, err
	}
	var xattrs []string
	...
	for len(data) > 0 {
		...
		if idx > 0 {
			xattrs = append(xattrs, string(data[:idx]))
		}
		data = data[idx+1:]
	}
	return xattrs, nil
...
func (i *directfsInode) mknod(ctx context.Context, name string, creds *auth.Credentials, opts *vfs.MknodOptions, d *dentry) (*dentry, error) {
	...
	// macOS doesn't have mknodat(2). For regular files, use openat with O_CREAT.
	fd, err := unix.Openat(i.controlFD, name, unix.O_CREAT|unix.O_EXCL|unix.O_WRONLY|hostOpenFlags, uint32(opts.Mode&^linux.FileTypeMask))
	if err != nil {
		return nil, err
	}
	_ = unix.Close(fd)
...
func (i *directfsInode) readlink() (string, error) {
	// macOS doesn't support readlinkat(fd, "") with AT_EMPTY_PATH semantics.
	// Use fcntl(F_GETPATH) to get the symlink's path, then Readlink on it.
	pathBuf := make([]byte, unix.PathMax)
	_, _, errno := unix.Syscall(unix.SYS_FCNTL, uintptr(i.controlFD), uintptr(unix.F_GETPATH), uintptr(unsafe.Pointer(&pathBuf[0])))
	if errno != 0 {
		return "", errno
	}
	// pathBuf is null-terminated.
	n := 0
	for n < len(pathBuf) && pathBuf[n] != 0 {
		n++
	}
	symPath := string(pathBuf[:n])

	for linkLen := 128; linkLen < math.MaxUint16; linkLen *= 2 {
		b := make([]byte, linkLen)
		n, err := unix.Readlink(symPath, b)
		if err != nil {
			return "", err
		}
		if n < linkLen {
...
func (i *directfsInode) statfs() (linux.Statfs, error) {
	...
	return linux.Statfs{
		...
		FragmentSize:    int64(statFS.Bsize),
		Blocks:          statFS.Blocks,
		BlocksFree:      statFS.Bfree,
		BlocksAvailable: statFS.Bavail,
		Files:           statFS.Files,
		FilesFree:       statFS.Ffree,
		// macOS Statfs_t does not have Namelen; use NAME_MAX (255).
		NameLength:      255,
	}, nil
}
...
func doRevalidationDirectfs(ctx context.Context, vfsObj *vfs.VirtualFilesystem, state *revalidateState, ds **[]*dentry) error {
	...
	for _, d := range state.dentries {
		// macOS doesn't have O_PATH; use tryOpen fallback.
		childFD, err := tryOpen(func(flags int) (int, error) {
			return unix.Openat(parent.controlFD, d.name, flags, 0)
		})
