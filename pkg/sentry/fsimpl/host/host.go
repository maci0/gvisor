...
func newInode(ctx context.Context, fs *filesystem, hostFD int, savable bool, restoreKey checkpoint.ResourceID, fileType linux.FileMode, isTTY bool, readonly bool) (*inode, error) {
	...
	if isTTY {
		...
		// For host TTY FDs, disable ECHO and ICANON in the virtual
		// termios so the guest shell handles its own echo and line
		// editing. The sentry doesn't implement a line discipline
		// for host FDs.
		i.termios.LocalFlags &^= linux.ECHO | linux.ECHOE | linux.ECHOK | linux.ECHOCTL | linux.ECHOKE | linux.ICANON
	}
