...
//
// On macOS, flags are ignored as preadv2 is not available. Single-block reads
// use pread(2), and multi-block reads use readv(2) (without offset) or
// sequential pread calls.
//
// Preconditions: !dsts.IsEmpty().
func Preadv2(fd int32, dsts safemem.BlockSeq, offset int64, flags uint32) (uint64, error) {
	// macOS does not have preadv2. For the single-block case, use read/pread.
	// For the multi-block case, use pread per-block sequentially since macOS
	// does not have preadv with offset.
	var (
		n uintptr
		e unix.Errno
	)
	if dsts.NumBlocks() == 1 {
		...
		if offset == -1 {
			...
			n, _, e = unix.Syscall6(unix.SYS_PREAD, uintptr(fd), dst.Addr(), uintptr(dst.Len()), uintptr(offset), 0, 0)
		}
	} else {
		n, e = iovecsReadWrite(true /*read*/, fd, safemem.IovecsFromBlockSeq(dsts), offset)
	}
	if e != 0 {
		return 0, e
	}
...
//
// On macOS, flags are ignored as pwritev2 is not available.
//
...
func Pwritev2(fd int32, srcs safemem.BlockSeq, offset int64, flags uint32) (uint64, error) {
	...
	if srcs.NumBlocks() == 1 {
		...
		if offset == -1 {
			...
			n, _, e = unix.Syscall6(unix.SYS_PWRITE, uintptr(fd), src.Addr(), uintptr(src.Len()), uintptr(offset), 0, 0)
		}
	} else {
		n, e = iovecsReadWrite(false /*read*/, fd, safemem.IovecsFromBlockSeq(srcs), offset)
	}
	if e != 0 {
		return 0, e
	}
