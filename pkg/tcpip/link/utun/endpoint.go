...
// InjectPacket implements PacketInjector for the utun endpoint.
// Prepends the 4-byte AF header and writes to the utun FD.
func (e *Endpoint) InjectPacket(ipPkt []byte) {
	buf := make([]byte, utunHeaderLen+len(ipPkt))
	binary.BigEndian.PutUint32(buf[:utunHeaderLen], unix.AF_INET)
	copy(buf[utunHeaderLen:], ipPkt)
	unix.Write(e.fd, buf)
}
...
func (e *Endpoint) EnableProxy(guestIP, hostIP net.IP) {
	e.nat = NewProxy(e, guestIP, hostIP)
}
