...
func (s *Socket) wait(write bool) error {
	for {
		...
		// macOS does not have ppoll; use poll(2) with an infinite timeout (-1).
		_, _, e := unix.Syscall(unix.SYS_POLL, uintptr(unsafe.Pointer(&events[0])), 2, uintptr(^uint32(0)))
		if e == unix.EINTR {
			continue
		}
		if e != 0 {
...
func (r *SocketReader) ReadVec(bufs [][]byte) (int, error) {
	...
	if len(r.ControlMessage) != 0 {
		...
		msg.SetControllen(len(r.ControlMessage))
	}
	...
	if msg.Controllen < uint32(len(r.ControlMessage)) {
...
func (w *SocketWriter) WriteVec(bufs [][]byte) (int, error) {
	...
	if len(w.ControlMessage) != 0 {
		...
		msg.SetControllen(len(w.ControlMessage))
	}
